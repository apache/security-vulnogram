...
// ASF
// we disable this for now because it triggers some validations
// that no longer hold
...
// And only keep the 'Public' variant, naming it 'Editor':
docSchema.oneOf.pop()
...
// ASF
// Keeps the legacy package identifiers (collectionURL + packageName) consistent with the Package URL.
//
// When a purl is present and maps to a known package collection,
// the two legacy fields are derived from it and disabled.
JSONEditor.defaults.editors.purlString = class purlString extends JSONEditor.defaults.editors.string {

    build() {
        super.build();
        // Commit point (blur, Enter, datalist pick). The stock handler is
        // registered first and unconditionally calls onChange, which has
        // already resolved the lock from the fresh value - only the write is
        // left to do here.
        this.input.addEventListener('change', () => {
            this.clearPurlInputTimer();
            const derived = purlToLegacyIdentifiers(this.getValue());
            if (derived) {
                this.writeLegacyIdentifiers(derived);
            }
        });
        // Also update as soon as what is typed becomes a usable purl - but
        // never unlock here, or the fields would flicker on every keystroke
        // that leaves the purl briefly invalid.
        this.input.addEventListener('input', () => {
            this.clearPurlInputTimer();
            this._purlInputTimer = setTimeout(() => {
                this._purlInputTimer = null;
                if (!this.parent) {
                    return;
                }
                const derived = purlToLegacyIdentifiers(this.input.value);
                if (derived) {
                    // The stock editor only commits a string on 'change', and
                    // onChange resolves the lock from the committed value.
    ...
    clearPurlInputTimer() {
        if (this._purlInputTimer) {
            clearTimeout(this._purlInputTimer);
            this._purlInputTimer = null;
        }
    }
    ...
    onChange(bubble, fromTemplate) {
        ...
        // Fires for programmatic writes too.
        // Locks legacy editors if their value is derivable from the Package URL.
        this.updateLegacyLock(purlToLegacyIdentifiers(this.getValue()));
    }

    // Returns the two sibling editors or null when either is unavailable.
    legacyEditors() {
        if (!this.parent || !this.parent.editors) {
            return null;
        }
        const collectionURL = this.parent.editors.collectionURL;
        const packageName = this.parent.editors.packageName;
        if (!collectionURL || !packageName) {
            return null;
        }
        return { collectionURL: collectionURL, packageName: packageName };
    }

    // Lock the legacy fields while a usable purl owns them, or hand them back.
    // Touches only the DOM, so it is safe on any change, user or programmatic.
    updateLegacyLock(locked) {
        const editors = this.legacyEditors();
        if (!editors) {
            return;
        }
        Object.values(editors).forEach((editor) => {
            if (locked) {
                editor.purlLocked = true;
                editor.disable(true);
            } else if (editor.purlLocked) {
                editor.purlLocked = false;
                editor.always_disabled = false;
                editor.enable();
            }
        });
    }

    // Derive the legacy fields from the purl and lock them. Only ever reached
    // from a user edit, so loading a record cannot rewrite what it stores.
    writeLegacyIdentifiers(derived) {
        const editors = this.legacyEditors();
        if (!editors) {
            return;
        }
        Object.keys(editors).forEach((key) => {
            editors[key].setValue(derived[key], false, false);
        });
    }
};
